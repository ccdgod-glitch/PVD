...
def generate_word_report(
  ...
  p1.add_run(
      f"ทำการติดตั้ง PVD ในชั้นดินอ่อนความหนาอัดตัว {H_soil:.1f} m"
      f" (ความหนาคำนวณทรุดตัว S_final = {H_sfinal:.1f} m) โดยจัดวางในรูปแบบ"
      f" {pattern} ที่ระยะห่าง S = {S:.2f} m"
  ...
  status_text = (
      ...
      if Uav_pct >= 90
      else "ยังไม่บรรลุตามเป้าหมาย (< 90%)"
  )
  ...
  results_data = [
      ...
      (
          "Degree of Consolidation แนวรัศมี (Ur)",
          f"{Ur_pct:.2f}%",
          "Barron / Hansbo",
      ),
      (
          "Degree of Consolidation เฉลี่ยรวม (Uav)",
          f"{Uav_pct:.2f}%",
          status_text,
      ),
      (
          "การทรุดตัวที่เกิดขึ้นแล้ว (St)",
          f"{S_target:.3f} m",
          f"จาก S_final = {S_final:.3f} m",
      ),
  ...
  for d_val in test_days:
    if d_val <= 365:
      ...
      ur_p = (
          1 - np.exp((-8 * (Cr * d_val) / (d_e_m * 100) ** 2) / Fn)
      ) * 100
      tv_tmp = (Cv * d_val) / ((H_soil * 100 / 2) ** 2)
      uv_tmp = (
          np.sqrt(4 * tv_tmp) / np.pi
          if tv_tmp <= 0.286
          else 1 - (10 ** (-0.085 - 0.933 * tv_tmp))
      )
  ...
  if Uav_pct >= 90.0:
    p3.add_run(
        f"สรุปการประเมิน: การติดตั้ง PVD ระยะห่าง {S:.2f} เมตร"
        " มีความเหมาะสมและเพียงพอทางการวิศวกรรม โดยชั้นดินสามารถคายน้ำและเกิดการทรุดตัวได้ถึง"
        f" {Uav_pct:.2f}% ภายในระยะเวลา {target_day} วัน (ใช้เวลาเพียง {days_90}"
        " วันในการบรรลุ U=90%) ซึ่งเป็นไปตามมาตรฐานการควบคุมการทรุดตัวก่อนการเปิดใช้งานพื้นที่"
    )
  else:
    p3.add_run(
        f"ข้อสังเกตทางการวิศวกรรม: การติดตั้ง PVD ที่ระยะห่าง {S:.2f} เมตร"
        f" ณ เวลา {target_day} วัน ทำให้เกิดการอัดตัวคายน้ำได้เพียง {Uav_pct:.2f}%"
        " ซึ่งยังไม่ถึงเกณฑ์ 90% จึงมีข้อเสนอแนะให้ปรับลดระยะห่างการติดตั้ง PVD (S)"
        f" ให้แคบลง เช่น ปรับลดเหลือ {max(0.6, S-0.2):.2f} เมตร"
        " หรือขยายระยะเวลาในการรอคอยการทรุดตัว (Preloading Period) เพิ่มเติม"
    )
...
# ---------------------------------------------------------
st.markdown(
    '<div class="main-header">🏗️ PVD Consolidation Analytics</div>',
    unsafe_allow_html=True,
)
st.markdown(
    '<div class="sub-header">ระบบจำลองการทรุดตัวและคำนวณเวลาการอัดตัวคายน้ำชั้นดินอ่อน'
    " (ปรับหน่วยตามตำราเรียน / สไลด์เลคเชอร์)</div>",
    unsafe_allow_html=True,
)
...
with st.sidebar.expander("📐 1. รูปแบบ PVD & ระยะติดตั้ง", expanded=True):
  ...
  S = st.number_input(
      "ระยะห่างการติดตั้ง S (m)",
      value=get_param("S", float, 1.00),
      step=0.05,
      format="%.2f",
      key="mem_S",
      on_change=update_url,
  )
  H_pvd = st.number_input(
      "ความลึกแผ่น PVD (m)",
      value=get_param("H_pvd", float, 10.0),
      step=0.5,
      key="mem_H_pvd",
      on_change=update_url,
  )
  a_mm = st.number_input(
      "ความกว้าง PVD: a (mm)",
      value=get_param("a_mm", float, 100.0),
      step=5.0,
      key="mem_a_mm",
      on_change=update_url,
  )
  b_mm = st.number_input(
      "ความหนา PVD: b (mm)",
      value=get_param("b_mm", float, 5.0),
      step=0.5,
      key="mem_b_mm",
      on_change=update_url,
  )
...
with st.sidebar.expander("🧪 2. คุณสมบัติชั้นดิน & ทางระบายน้ำ", expanded=True):
  H_soil = st.number_input(
      "ความหนาชั้นดินอ่อน: H_soil (m)",
      value=get_param("H_soil", float, 30.0),
      step=1.0,
      key="mem_H_soil",
      on_change=update_url,
  )
  ...
  drain_options = [
      "ระบายน้ำ 2 ทาง (บน-ล่าง: Hd = H/2)",
      "ระบายน้ำทางเดียว (Hd = H)",
  ]
  def_drain = get_param("drainage", str, drain_options[0])
  drainage_type = st.radio(
      "ลักษณะการระบายน้ำแนวดิ่ง (Drainage Path)",
      drain_options,
  ...
  if "cm²/day" in unit_choice:
    Cv_input = st.number_input(
        "ค่า Cv (cm²/day)",
        value=get_param("Cv", float, 20.0),
        step=1.0,
        key="mem_Cv",
        on_change=update_url,
    )
    Cr_input = st.number_input(
        "ค่า Cr (cm²/day)",
        value=get_param("Cr", float, 140.0),
        step=5.0,
        key="mem_Cr",
        on_change=update_url,
    )
    ...
    Cv_m2_yr = st.number_input(
        "ค่า Cv (m²/year)",
        value=get_param("Cv", float, 2.0),
        step=0.1,
        key="mem_Cv",
        on_change=update_url,
    )
    Cr_m2_yr = st.number_input(
        "ค่า Cr (m²/year)",
        value=get_param("Cr", float, 6.0),
        step=0.1,
        key="mem_Cr",
        on_change=update_url,
    )
  ...
  st.markdown("---")
  use_lab_data = st.checkbox(
      "คำนวณ e₀ จากผลทดสอบ Lab",
      value=get_param("uselab", bool, False),
      key="mem_uselab",
      on_change=update_url,
  )
  if use_lab_data:
    Gs = st.number_input(
        "Specific Gravity (Gs)",
        value=get_param("Gs", float, 2.65),
        step=0.01,
        key="mem_Gs",
        on_change=update_url,
    )
    w_wet = st.number_input(
        "น้ำหนักดินเปียก W_wet (g)",
        value=get_param("wwet", float, 120.0),
        step=1.0,
        key="mem_wwet",
        on_change=update_url,
    )
    w_dry = st.number_input(
        "น้ำหนักดินแห้ง W_dry (g)",
        value=get_param("wdry", float, 80.0),
        step=1.0,
        key="mem_wdry",
        on_change=update_url,
    )
    V_cm3 = st.number_input(
        "ปริมาตรวงแหวน V (cm³)",
        value=get_param("Vcm3", float, 60.0),
        step=1.0,
        key="mem_Vcm3",
        on_change=update_url,
    )
  ...
  sigma_0 = st.number_input(
      "Effective Stress เดิม: σ0' (kPa)",
      value=get_param("sigma_0", float, 50.0),
      step=5.0,
      key="mem_sigma_0",
      on_change=update_url,
  )
  delta_sigma = st.number_input(
      "น้ำหนักถมเพิ่ม: Δσ (kPa)",
      value=get_param("delta_sigma", float, 80.0),
      step=5.0,
      key="mem_delta_sigma",
      on_change=update_url,
  )
...
with st.sidebar.expander("🎯 3. กำหนดวันเป้าหมาย & Smear Effect", expanded=True):
  target_day = st.number_input(
      "วันเป้าหมายหลักที่ต้องการตรวจสอบ (วัน)",
      value=get_param("target_day", int, 90),
      step=10,
      min_value=1,
      key="mem_target_day",
      on_change=update_url,
  )
  include_smear = st.checkbox(
      "คิดผลกระทบ Smear Effect",
      value=get_param("smear", bool, False),
      key="mem_smear",
      on_change=update_url,
  )
  if include_smear:
    d_s_ratio = st.number_input(
        "อัตราส่วน ds / dw",
        value=get_param("ds_ratio", float, 2.5),
        step=0.1,
        key="mem_ds_ratio",
        on_change=update_url,
    )
    kh_ks_ratio = st.number_input(
        "อัตราส่วน kh / ks",
        value=get_param("kh_ks", float, 3.0),
        step=0.5,
        key="mem_kh_ks",
        on_change=update_url,
    )
...
with st.sidebar.expander("🏖️ 4. Sand Mat (ชั้นทรายซับน้ำ)", expanded=True):
  include_sandmat = st.checkbox(
      "คิดผลกระทบความต้านทาน Sand Mat",
      value=get_param("sandmat", bool, False),
      key="mem_sandmat",
      on_change=update_url,
  )
  if include_sandmat:
...
F_n_eff = (
    Fn + (kh_ks_ratio - 1) * np.log(d_s_ratio) if include_smear else Fn
)
...
S_final = (
    H_sfinal
    * (Cc / (1.0 + e0))
    * np.log10((sigma_0 + delta_sigma) / sigma_0)
)

t_day = float(target_day)
Tv = (Cv_cm2_day * t_day) / (H_d_cm**2)
Uv = (
    (2.0 * np.sqrt(Tv)) / np.sqrt(np.pi)
    if Tv <= 0.282
    else 1.0 - (10 ** (-(Tv + 0.085) / 0.933))
)
...
for d_day in days:
  ...
  U_v_i = (
      (2.0 * np.sqrt(Tv_i)) / np.sqrt(np.pi)
      if Tv_i <= 0.282
      else 1.0 - (10 ** (-(Tv_i + 0.085) / 0.933))
  )
...
m4.metric(
    ...
    delta_color=(
        "normal" if isinstance(days_90, (int, np.integer)) else "inverse"
    ),
)
...
with tab_steps:
  st.markdown(
      f"### 📑 ตารางคำนวณและตรวจสอบที่ระยะ $S = {S:.2f}$ m ณ เวลา $t ="
      f" {target_day}$ วัน"
  )
  ...
  for s_val in s_test_list:
    ...
    fn_rows.append({
        ...
        "(7) (3n²-1)/(4n²)": f"{t3:.3f}",
        "(8) F(n) = (5)×(6)-(7)": (
            f"★ {fn_val:.3f}" if s_val == S else f"{fn_val:.3f}"
        ),
    })
  st.table(pd.DataFrame(fn_rows))
  st.info(
      f"💡 **ผลลัพธ์ข้อ ⑤:** ที่ระยะติดตั้ง $S = {S:.2f}$ m"
  ...
  st.markdown(
      ...
      f" {target_day}$ วัน (Theory of Terzaghi)"
  )
  ...
  with col_v1:
    ...
    st.latex(
        r"U_v = \frac{\sqrt{4 \times T_v}}{\pi} \quad (\text{เมื่อ } U_v \le"
        r" 60\%)"
    )
    st.latex(
        r"U_v = 1 - 10^{-0.085 - 0.933 T_v} \quad (\text{เมื่อ } U_v > 60\%)"
    )

  with col_v2:
    Tv_target = (Cv_cm2_day * target_day) / (H_d_cm**2)
    Uv_target = (
        np.sqrt(4 * Tv_target) / np.pi
        if Tv_target <= 0.286
        else 1 - (10 ** (-0.085 - 0.933 * Tv_target))
    )
    Uv_target_pct = min(Uv_target * 100.0, 100.0)

    st.write(f"**แสดงการแทนค่าตัวเลข ณ เวลาเป้าหมาย $t = {target_day}$ วัน:**")
    st.write(
        f"- ค่าสัมประสิทธิ์การอัดตัว $C_v = \\mathbf{{{Cv_cm2_day:.1f}}}$"
        " cm²/day"
    )
    st.write(
        f"- ความหนาชั้นดิน $H = {H_soil}$ m ➔ ระยะระบายน้ำ $H_d ="
        f" \\mathbf{{{H_d_cm:.0f}}}$ cm ({drainage_type.split(' ')[0]})"
    )
    st.write(
        f"- คำนวณ Time Factor: $T_v = \\frac{{{Cv_cm2_day:.1f} \\times"
        f" {target_day}}}{{({H_d_cm:.0f})^2}} = \\mathbf{{{Tv_target:.4f}}}$"
    )
    st.write(
        f"- **ผลลัพธ์ข้อ ⑦ ได้ค่า $U_v$:**"
  ...
  Uav_target_pct = 100.0 * (
      1.0 - (1.0 - (Ur_target / 100.0)) * (1.0 - (Uv_target_pct / 100.0))
  )

  st.write(f"**แทนค่าที่เวลา $t = {target_day}$ วัน:**")
  st.latex(
      f"U_{{av}} = 1 - (1 - {Ur_target/100:.4f})(1 -"
      f" {Uv_target_pct/100:.4f}) = \\mathbf{{{Uav_target_pct:.2f}\\%}}"
  )
  ...
  for d_val in sorted(list(set([30, 60, target_day, 180, 270, 365]))):
    ...
    summary_rows.append({
        ...
        "⑧ U_av รวม = 1-(1-Ur)(1-Uv)": (
            f"★ {uav_p:.2f}%" if d_val == target_day else f"{uav_p:.2f}%"
        ),
  ...
  if Uav_target_pct >= 90.0:
    st.success(
        f"🎯 **บทสรุปข้อ ⑨:** ที่ระยะติดตั้ง PVD **S = {S:.2f} เมตร** ชั้นดินสามารถอัดตัวคายน้ำได้ **{Uav_target_pct:.2f}%** ณ วันที่ {target_day} ซึ่ง **ผ่านเกณฑ์ (> 90%)** เป็นไปตามข้อกำหนดการออกแบบครับ"
    )
  else:
    st.warning(
  ...
  with col_sf1:
    st.latex(
        r"S_{final} = H_{sfinal} \cdot \frac{C_c}{1 + e_0} \cdot"
        r" \log_{10}\left(\frac{\sigma_0' + \Delta\sigma}{\sigma_0'}\right)"
    )
  with col_sf2:
    st.write("**แสดงการแทนค่าตัวเลขจากพารามิเตอร์ที่กำหนด:**")
    st.latex(
        f"S_{{final}} = {H_sfinal:.2f} \\cdot \\frac{{{Cc:.3f}}}{{1 +"
        f" {e0:.3f}}} \\cdot"
        f" \\log_{{10}}\\left(\\frac{{{sigma_0:.2f} +"
        f" {delta_sigma:.2f}}}{{{sigma_0:.2f}}}\\right)"
    )
    st.write(
        f"- 💡 **ความหนาชั้นดินคำนวณทรุดตัว ($H_{{sfinal}}$):**"
        f" $\\mathbf{{{H_sfinal:.2f}}}$ **เมตร**"
    )
    st.write(
        f"- 💡 **การทรุดตัวสูงสุด ($S_{{final}}$):**"
        f" $\\mathbf{{{S_final:.4f}}}$ **เมตร** (หรือ"
        f" `{S_final*100:.2f}` cm)"
    )
    st.write(
        f"- 💡 **การทรุดตัว ณ วันที่ {target_day} ($S_t = U_{{av}} \\times"
        f" S_{{final}}$):** $\\mathbf{{{St:.4f}}}$ **เมตร** (หรือ"
        f" `{St*100:.2f}` cm)"
    )
  ...
  with col_right:
    ...
    fig_u.add_trace(
        go.Scatter(
            x=df["Day"],
            y=df["U_v"],
            name="แนวดิ่ง ดิน (U_v)",
            line=dict(color="#9CA3AF", dash="dot"),
        )
    )
    fig_u.add_hline(
        y=90, line_dash="dash", line_color="#F59E0B", annotation_text="Target 90%"
    )
    fig_u.update_layout(
...
with tab_data:
  ...
  st.download_button(
      ...
      data=csv,
      file_name="pvd_consolidation_result.csv",
      mime="text/csv",
  )
  st.dataframe(
      df.style.format({
...
with tab_summary:
  st.success(
      f"**สรุปผลการประเมิน:** ในระยะติดตั้ง PVD ที่ **{S:.2f} เมตร** รูปแบบ"
      f" **{pattern}**"
  )
  st.write(
      f"- การทรุดตัวทั้งหมดเมื่อสิ้นสุดกระบวนการ ($S_{{final}}$ จาก"
      f" $H_{{sfinal}} = {H_sfinal:.1f}$ m): **{S_final:.3f} เมตร**"
  )
  if isinstance(days_90, (int, np.integer)):
    st.write(f"- ดินจะทรุดตัวถึง 90% ภายในเวลา **{days_90} วัน**")
  else:
    st.warning(
        "- การติดตั้ง PVD ระยะนี้ยังไม่สามารถทำให้ดินทรุดตัวถึง 90% ได้ภายใน 1"
        " ปี แนะนำให้ **ลดระยะห่าง S** ลง"
    )

  st.markdown("---")
  st.markdown(
      "#### 📄 ส่งออกรายงานสรุปผลพร้อมตารางคำนวณ (Word Document)"
  )
  st.write(
      "กดปุ่มด้านล่างเพื่อดาวน์โหลดไฟล์เอกสาร `.docx` สรุปผลพร้อมตารางคำนวณประกอบรายงาน"
  )
  ...
  st.download_button(
      label=(
          "📥 ดาวน์โหลดรายงานสรุปผลพร้อมตาราง (Word File .docx)"
      ),
      data=doc_file,
      file_name=(
          f"PVD_Engineering_Report_S{S}m_{target_day}days.docx"
      ),
      mime=(
